...
def get_sidebar(schema, sidebar_content):
    ...
    for table_name in schema.keys():
        label = "<i class='fa fa-table'></i> " + table_name.replace("_"," ").capitalize()
        table_links.append(pn.pane.HTML(f"<div style='padding:4px 8px;font-size:14px;cursor:pointer;'>{label}</div>"))

    return pn.Column("### Performance Tables", *table_links, width=300)
    ...
    def tasks_to_charts(clicked_button, tasks):
        for task in tasks:
            if task.w_task.value == 'timeseries analysis':
                ...
                value_name = task.w_data_multi.value[0]
                if value_name == "disk_write_bytes":
                    labels = {"value": "bytes"}
                elif "mem" in value_name:
                    labels = {"value": "bytes"}
                elif "percent" in value_name:
                    labels = {"value": "percentage"}
                elif "latency" in value_name:
                    labels = {"value": "seconds"}
                elif "core" in value_name or 'sys' in value_name:
                    labels = {"value": "percentage"}
                else:
                    labels = None
                dashboard = metrics_task_viz_template(y=task.w_data_multi.value, chart_type = 'line', label=labels).plot(df)
...
class base_task_viz_template:
    ...
    def plot(self, df, derived_df = None, title = ""):
        ...
        if self.chart_type == 'scatter':
            ...
            fig = px.line(df, x=self.x, y=self.y, color=self.color, facet_col=self.col, facet_row = self.row, labels=self.label)
...
class metrics_task_viz_template(base_task_viz_template):
    def __init__(self, y, chart_type, color=None, shape=None, pattern=None, size=None, row=None, col=None, tab=None, legend=None, label=None):
        super().__init__(chart_type, label=label)
        self.x = 'timestamp'
        self.y = y
        self.chart_type = chart_type
...
class anomaly_detection_and_explanation_task_viz_template(base_task_viz_template):
    ...
    def plot(self, df, df_task):
        ...
        for i, index in enumerate(anomaly_indices):
            ...
            # Get text (좌우 같을 경우 text 제외)
            
            prev_index = anomaly_indices[i-1] if i-1 > 0 else None
            next_index = anomaly_indices[i+1] if i+1 < len(anomaly_indices) else None
            is_different_with_prev = prev_index is not None and (df_task["anomaly_cause"][index] != df_task["anomaly_cause"][prev_index])
            is_different_with_next = next_index is not None and (df_task["anomaly_cause"][index] != df_task["anomaly_cause"][next_index])
            if prev_index is None or next_index is None or is_different_with_prev or is_different_with_next:
                annotation_func = partial(go.layout.Annotation, 
            ...
            annotation = annotation_func(
                    x=df["timestamp"][index],
                    y=df[self.y][index],
                )
            # if False and (index-1 > 0 and df_task["anomaly_cause"][index] != df_task["anomaly_cause"][index-1]) or (index+1 < len(df_task["anomaly_cause"]) and df_task["anomaly_cause"][index] != df_task["anomaly_cause"][index+1]):
