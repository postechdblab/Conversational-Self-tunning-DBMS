...
class WarmupPolynomialLRScheduler:
    ...
    def assert_correct_param_order(self):
        """
        Check non_bert parameter comes first
        """
        return True
...
class BertAdamW(transformers.AdamW):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        The order of params list is important.
        That is, non_bert should comes first for consistent lr for lr_scheduler
        """
        params = []
        self.non_bert_param_group = {"params": non_bert_params}
        params.append(self.non_bert_param_group)
        if not freeze_bert:
            ...
            params.append(self.bert_param_group)
        if "name" in kwargs:
...
class BertWarmupPolynomialLRSchedulerGroup(WarmupPolynomialLRScheduler):
    ...
    def assert_correct_param_order(self):
        ...
        if len(self.param_groups) > 1:
            ...
            lens = [len(group["params"]) for group in self.param_groups]
