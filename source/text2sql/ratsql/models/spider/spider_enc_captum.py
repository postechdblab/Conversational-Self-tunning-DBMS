...
from source.text2sql.ratsql.models import abstract_preproc
from source.text2sql.ratsql.models.spider import spider_enc_modules
from source.text2sql.ratsql.models.spider.spider_match_utils import (
    compute_schema_linking,
    compute_cell_value_linking,
)
from source.text2sql.ratsql.resources import corenlp
from source.text2sql.ratsql.utils import registry
from source.text2sql.ratsql.utils import serialization
from source.text2sql.ratsql.utils import vocab
...
class SpiderEncoderState:
    ...
    return_dic = attr.ib(default=None)
...
class PreprocessedSchema:
    ...
    col_descs = attr.ib(factory=list)
...
def preprocess_schema_uncached(
    ...
    for i, column in enumerate(schema.columns):
        ...
        if use_column_description:
            col_desc_toks = tokenize_func([], column.description)
            r.col_descs.append(col_desc_toks)
        ...
        if bert:
            ...
            if use_column_description:
                col_toks_with_coldesc = deepcopy(col_toks)
                col_toks_with_coldesc.append(".")
                col_toks_with_coldesc += col_desc_toks
                r.normalized_column_names.append(
                    Bertokens(col_toks_with_coldesc, bert_version=bert_version)
                )
            else:
                r.normalized_column_names.append(
                    Bertokens(col_toks, bert_version=bert_version)
                )
        else:
    ...
    for i, table in enumerate(schema.tables):
        ...
        if bert:
            r.normalized_table_names.append(
                Bertokens(table_toks, bert_version=bert_version)
            )
    ...
    r.primary_keys = (
        ...
        if fix_issue_16_primary_keys
            ...
            column.id for column in last_table.primary_keys for table in schema.tables
        ]
    )
...
class SpiderEncoderV2Preproc(abstract_preproc.AbstractPreproc):
    ...
    def add_item(
        ...
        preprocessed = self.preprocess_item(
            item, validation_info, manual_linking_info=manual_linking_info
        )
    ...
    def preprocess_item(self, item, validation_info, manual_linking_info=None):
        ...
        if self.compute_sc_link:
            ...
            column_names_without_types = [
                col[1:] for col in preproc_schema.column_names
            ]
        ...
        if self.compute_cv_link:
            cv_link = compute_cell_value_linking(
                question, item.schema, manual_linking_info=manual_linking_info
            )
        else:
...
class SpiderEncoderV2(torch.nn.Module):
    ...
    def _build_modules(self, module_types):
        module_builder = {
            ...
            "linear": lambda: spider_enc_modules.EmbLinear(
                input_size=self.word_emb_size, output_size=self.word_emb_size
            ),
    ...
    def forward(self, descs, debug=False):
        ...
        for batch_idx, desc in enumerate(descs):
            ...
            if debug:
                assert len(descs) == 1, "batch computation not considered"
                # Create new token list based on the
                # Tokens
                # Columns
                # Tables
                return_dic["question"] = desc["question"]
                return_dic["columns"] = desc["columns"]
                return_dic["tables"] = desc["tables"]
...
class Bertokens:
        ...
        if self.bert_version == "markussagen/xlm-roberta-longformer-base-4096":
            ...
            for idx, tok in enumerate(self.pieces):
                ...
                if is_english_start(tok):
                    ...
                    if new_toks and any(
                        [func(new_toks[-1]) for func in [is_english, is_dash]]
                    ):
                    ...
                    if new_toks and any(
                        [func(new_toks[-1]) for func in [is_english, is_int, is_dash]]
                    ):
                        new_toks[-1] = new_toks[-1] + tok
                    else:
                        idx_map[len(new_toks)] = idx
                    ...
                    if new_toks and any(
                        [func(new_toks[-1]) for func in [is_english, is_int]]
                    ):
                        new_toks[-1] = new_toks[-1] + tok
                    else:
                        idx_map[len(new_toks)] = idx
            ...
            for idx, tok in enumerate(self.pieces):
                if not new_toks:
                    ...
                    new_toks[-1] = new_toks[-1] + tok
            self.idx_map = idx_map
        elif self.bert_version == "allenai/longformer-large-4096":
            split_indicator = "Ġ"
            new_toks = []
            idx_map = {0: 0}

            tmp = self.pieces[0]
            for idx, tok in enumerate(self.pieces[1:]):
                real_idx = idx + 1
                if tok[0] == split_indicator:
                    new_toks += [tmp]
                    tmp = tok.strip(split_indicator)
                    idx_map[len(new_toks)] = real_idx
                else:
                    tmp += tok
            self.idx_map = idx_map
        else:
    ...
    def bert_schema_linking(self, columns, tables, manual_linking_info=None):
        ...
        sc_link = compute_schema_linking(
            question_tokens, column_tokens, table_tokens, manual_linking_info
        )
...
class SpiderEncoderBertPreproc(SpiderEncoderV2Preproc):
    ...
    def __init__(
        ...
        self.compute_sc_link = compute_sc_link
        self.compute_cv_link = compute_cv_link
        self.use_column_description = use_column_description
        # JHCHO - 21.11.02: Add feature for using col-desc embedding
        self.use_column_desc_emb = use_column_desc_emb & use_column_description
        self.use_kor_nng_translate = use_kor_nng_translate
        self.use_kor_full_translate = use_kor_full_translate
        ...
        self.max_position_embeddings = transformers.AutoConfig.from_pretrained(
            bert_version
        ).max_position_embeddings

        # TODO: should get types from the data
        column_types = ["text", "number", "time", "boolean", "others", "etc"]
        self.tokenizer.add_tokens([f"<type: {t}>" for t in column_types])

        if self.use_kor_nng_translate:
            self.pos_tag = Kkma().pos
    ...
    def translate(self, *args, **kwargs):
        ...
        """
        assert (
            self.use_kor_nng_translate or self.use_kor_full_translate
        ), "Check your config"
        if self._translate == None:
            self._translate = transformers.pipeline(
                "translation_ko_to_en", model="Helsinki-NLP/opus-mt-ko-en", device=0
            )
    ...
    def preprocess_item(
        ...
        def replace_NNG_with_eng(pos_tagged_question, ori_text):
            sent = []
            assert len(pos_tagged_question) == len(ori_text)
            for idx, tags in enumerate(pos_tagged_question):
                # Check if all words are preserved
                if all(word in ori_text[idx] for word, pos in tags):
                    words = []
                    for word, pos in tags:
                        ...
                        if pos != "NNG":
                            ...
                            continue
                        # Translate Kor -> Eng
                        tranlated_entity = self.translate(word, return_text=True)[0][
                            "translation_text"
                        ]
                        # Append Eng if the word is in the schema
                        if word_is_in_schema(tranlated_entity, item.schema):
                            words.append(tranlated_entity)
                        else:
                            words.append(word)
                else:
                    words = ori_text[idx]
        ...
        if self.use_kor_nng_translate:
            # Perform POS tagging
            pos_tagged_question = self.pos_tag(item.orig["question"], flatten=False)
            # If original string can be preserved with pos information
            if len(pos_tagged_question) == len(item.text):
                # Replace kor -> eng if POS tag is "NNG"
                replaced_toks = replace_NNG_with_eng(pos_tagged_question, item.text)
                replaced_text = " ".join(replaced_toks)
                item.orig["question"] = replaced_text
        elif self.use_kor_full_translate:
            item.orig["question"] = self.translate(
                item.orig["question"], return_text=True
            )[0]["translation_text"]
        ...
        if self.compute_cv_link:
            cv_link = question_bert_tokens.bert_cv_linking(
                item.schema, manual_linking_info
            )
        else:
            cv_link = {"num_date_match": {}, "cell_match": {}}
        ...
        return {
            ...
            "foreign_keys_tables": preproc_schema.foreign_keys_tables,
            "primary_keys": preproc_schema.primary_keys,
            "col_descs": preproc_schema.col_descs,
        }
    ...
    def validate_item(self, item, section, bert_version="bert-base-uncased"):
        ...
        num_words = (
            ...
            + (
                len(preproc_schema.column_names)
                if self.use_column_desc_emb
                else sum(len(d) + 1 for d in preproc_schema.col_descs)
            )
        )
    ...
    def _preprocess_schema(self, schema, bert_version="bert-base-uncased"):
        ...
        result = preprocess_schema_uncached(
            ...
            bert=True,
            bert_version=bert_version,
            use_column_description=self.use_column_description,
        )
        self.preprocessed_schemas[schema.db_id] = result
        return result
    ...
    def load(self):
        ...
        self.tokenizer = transformers.AutoTokenizer.from_pretrained(
            self.data_dir,
            config=transformers.AutoConfig.from_pretrained(self.bert_version),
        )


@registry.register("encoder", "spider-captum")
class SpiderEncoderBert(torch.nn.Module):
    ...
    def __init__(
        ...
        # JHCHO - 21.11.02: Add column embedding bert model
        self.bert_col_emb_model = transformers.AutoModel.from_pretrained(bert_version)
        for param in self.bert_col_emb_model.parameters():
            param.requires_grad = False

        self.tokenizer = self.preproc.tokenizer
        # # JHCHO - 21.11.02: Add special token for column description
        # self.tokenizer.add_special_tokens({'additional_special_tokens':['[REPLACE]']})
    ...
    def forward(self, descs, input, att_masks, tok_type_lists, debug=False):
        REMOVE_SCHEMA_LINK = False
        if REMOVE_SCHEMA_LINK:
            for desc in descs:
                desc["sc_link"] = {"q_col_match": {}, "q_tab_match": {}}
                desc["cv_link"] = {"num_date_match": {}, "cell_match": {}}
        batch_token_lists = []
        batch_col_desc_emb_lists = []
        ...
        for batch_idx, desc in enumerate(descs):
            ...
            for i, c in enumerate(desc["columns"]):
                if self.use_column_type:
                    ...
                    col = c[:-1]
                if self.preproc.use_column_description:
                    # JHCHO - 21.11.02: Add [REPLACE] token for col-desc emb
                    if self.preproc.use_column_desc_emb:
                        col += ["[REPLACE]"]
                        col_desc = self.pad_single_sentence_for_bert(
                            desc["col_descs"][i], cls=True
                        )
                        indexed_col_desc = self.tokenizer.convert_tokens_to_ids(
                            col_desc
                        )
                        batch_col_descs.append(indexed_col_desc)
                    else:
                        col += ["."] + desc["col_descs"][i]
                cols.append(self.pad_single_sentence_for_bert(col, cls=False))

            tabs = [
                self.pad_single_sentence_for_bert(t, cls=False) for t in desc["tables"]
            ]
            ...
            if len(token_list) > self.bert_model.config.max_position_embeddings:
                long_seq_set.add(batch_idx)
                continue

            # JHCHO - 21.11.02: Generate col-desc emb
            if self.preproc.use_column_description and self.preproc.use_column_desc_emb:
                (
                    padded_col_desc_token_lists,
                    col_desc_att_mask_lists,
                    col_desc_tok_type_lists,
                ) = self.pad_sequence_for_bert_batch(batch_col_descs)
                col_desc_tokens_tensor = torch.tensor(
                    padded_col_desc_token_lists, dtype=torch.long, device=self._device
                )
                col_desc_att_masks_tensor = torch.tensor(
                    col_desc_att_mask_lists, dtype=torch.long, device=self._device
                )
                if not isinstance(
                    self.bert_col_emb_model, transformers.XLMRobertaModel
                ):
                    col_desc_tok_type_tensor = torch.tensor(
                        col_desc_tok_type_lists, dtype=torch.long, device=self._device
                    )
                else:
                    col_desc_tok_type_tensor = None

                col_desc_output = self.bert_col_emb_model(
                    col_desc_tokens_tensor,
                    attention_mask=col_desc_att_masks_tensor,
                    token_type_ids=col_desc_tok_type_tensor,
                )[0]
                batch_col_desc_emb_lists.append(
                    col_desc_output[:, 0]
                )  # Get [CLS] embedding
            ...
            question_rep_ids = torch.tensor(
                question_indexes, dtype=torch.long, device=self._device
            )
            batch_id_to_retrieve_question.append(question_rep_ids)
            column_rep_ids = torch.tensor(
                column_indexes, dtype=torch.long, device=self._device
            )
            batch_id_to_retrieve_column.append(column_rep_ids)
            table_rep_ids = torch.tensor(
                table_indexes, dtype=torch.long, device=self._device
            )
            batch_id_to_retrieve_table.append(table_rep_ids)
            if self.summarize_header == "avg":
                assert all(i2 >= i1 for i1, i2 in zip(column_indexes, column_indexes_2))
                column_rep_ids_2 = torch.tensor(
                    column_indexes_2, dtype=torch.long, device=self._device
                )
                batch_id_to_retrieve_column_2.append(column_rep_ids_2)
                assert all(i2 >= i1 for i1, i2 in zip(table_indexes, table_indexes_2))
                table_rep_ids_2 = torch.tensor(
                    table_indexes_2, dtype=torch.long, device=self._device
                )
        ...
        input = input
        att_masks = att_masks
        tok_type_tensor = tok_type_lists

        padded_token_lists, att_mask_lists, tok_type_lists = (
            self.pad_sequence_for_bert_batch(batch_token_lists)
        )
        tokens_tensor = torch.tensor(
            padded_token_lists, dtype=torch.long, device=self._device
        )
        att_masks_tensor = torch.tensor(
            att_mask_lists, dtype=torch.long, device=self._device
        )

        # JHCHO - 21.11.02: Replace [REPLACE] token with col-desc embedding
        if self.preproc.use_column_description and self.preproc.use_column_desc_emb:
            inputs_embeds = self.bert_model.embeddings.word_embeddings(tokens_tensor)

            for batch_num in range(tokens_tensor.size(0)):
                input_ids_single_batch = tokens_tensor[batch_num]
                col_desc_embedding_iter = iter(batch_col_desc_emb_lists[batch_num])
                for token_num in range(input_ids_single_batch.size(0)):
                    input_id = input_ids_single_batch[token_num]
                    if input_id == self.tokenizer.additional_special_tokens_ids[0]:
                        inputs_embeds[batch_num][token_num] = next(
                            col_desc_embedding_iter
                        )

            if self.bert_token_type:
                # if not isinstance(self.bert_model, transformers.XLMRobertaModel):
                ...
                if debug:
                    ...
                    bert_attentions = bert_output.attentions
                ...
                bert_output = self.bert_model(
                    None, attention_mask=att_masks_tensor, inputs_embeds=inputs_embeds
                )[0]
        else:
            if self.bert_token_type:
                # if not isinstance(self.bert_model, transformers.XLMRobertaModel) and not self.bert_model.base_model_prefix == 'longformer':
                #     tok_type_tensor = torch.tensor(tok_type_lists, dtype=torch.long, device=self._device)
                # else:
                #     tok_type_tensor = None
                ...
                if debug:
                    ...
                    bert_output = self.bert_model(
                        input, attention_mask=att_masks, token_type_ids=tok_type_tensor
                    )[0]
            else:
                bert_output = self.bert_model(input, attention_mask=att_masks)[0]
        ...
        for batch_idx, desc in enumerate(descs):
            ...
            if debug:
                ...
                return_dic["columns"] = desc["columns"]
                return_dic["tables"] = desc["tables"]
                return_dic["input_tokens"] = token_list
                return_dic["bert_attention"] = bert_attentions
    ...
    def _bert_encode(self, toks):
        if not isinstance(toks[0], list):  # encode question words
            ...
            for item_toks in toks:
                item_toks = item_toks + [self.tokenizer.pad_token] * (
