        ...
        if self.preproc.use_column_description and self.preproc.use_column_desc_emb:
            ...
            if self.bert_token_type:
                if not isinstance(self.bert_model, transformers.XLMRobertaModel):
                    tok_type_tensor = torch.tensor(
                        tok_type_lists, dtype=torch.long, device=self._device
                    )
                else:
            ...
            if self.bert_token_type:
                if (
                    not isinstance(self.bert_model, transformers.XLMRobertaModel)
                    and not self.bert_model.base_model_prefix == "longformer"
                ):
                    tok_type_tensor = torch.tensor(
                        tok_type_lists, dtype=torch.long, device=self._device
                    )
