...
from source.text2sql.ratsql.datasets.spider import load_tables
from source.text2sql.ratsql.datasets.spider_lib import evaluation
from source.text2sql.ratsql.datasets.spider_lib.process_sql import (
    Schema,
    get_schema,
    get_sql,
)
from source.text2sql.ratsql.datasets.utils import db_utils
from source.text2sql.ratsql.grammars.postgres import PostgresLanguage
from source.text2sql.ratsql.models import abstract_preproc, attention, variational_lstm
from source.text2sql.ratsql.models.nl2code.infer_tree_traversal import (
    InferenceTreeTraversal,
)
from source.text2sql.ratsql.models.nl2code.train_tree_traversal import (
    TrainTreeTraversal,
)
from source.text2sql.ratsql.models.nl2code.tree_traversal import TreeTraversal
from source.text2sql.ratsql.utils import registry, serialization, vocab
...
class NL2CodeDecoderPreproc(abstract_preproc.AbstractPreproc):
    def __init__(
        ...
        # # Custom
        # if not table_path:
        #     table_path = str(Path(save_path).parent / "tables.json")
        # db_names = [item['db_id'] for item in json.load(open(table_path))]
        # self.schemas = {}
        # for db_name in db_names:
        #     db_conn_str = db_utils.create_db_conn_str(db_path, db_name, db_type=db_type)
        #     print(db_conn_str)
        #     if db_name == 'samsung':
        #         self.schemas[db_name] = Schema(get_schema("user=hkkang password=hkkang host=localhost dbname=samsung", db_type='postgres'))
        #     else:
    ...
    def validate_item(self, item, section):
        ...
        if parsed:
            ...
            if section not in {"train", "aux"}:
                schema = self.schemas[item.orig["db_id"]]
                query = item.orig["query"]
                try:
                    spider_json_sql = get_sql(
                        schema, query, grammar=self.grammar_identifier
                    )
                except:
                    print(f"Error in spider json processing!:{item.orig['query']}")
                    return False, None
            return True, parsed
        return section not in {"train", "aux"}, None

    def add_item(self, item, section, validation_info):
        root = validation_info
        if section in {"train", "aux"}:
    ...
    def add_item_from_cache(self, item, section):
        root, code = item
        if section in ["train", "val", "aux"]:
            for token in self._all_tokens(root):
                self.vocab_builder.add_word(token)
            self._record_productions(root)
        else:
            print(
                f"Note that data from {section} is not added to possible production rules!"
            )

        # Add NL2CodeDecoderP
        if section in {"train", "val"}:
            self.items[section].append(
                NL2CodeDecoderPreprocItem(tree=root, orig_code=code)
            )
    ...
    def load(self, checkpoint_path=None):
        if checkpoint_path and os.path.exists(checkpoint_path):
            ...
            key_list = [
                ...
                "field_presence_infos",
                "seq_lengths",
                "primitive_types",
                "all_rules",
                "rules_mask",
            ]
            if "custom" in checkpoint and all(
                key in checkpoint["custom"] for key in key_list
            ):
                self.vocab = vocab.Vocab(
                    checkpoint["custom"]["vocab"], special_elems=()
                )
                self.sum_type_constructors = checkpoint["custom"][
                    "sum_type_constructors"
                ]
                self.field_presence_infos = checkpoint["custom"]["field_presence_infos"]
                self.seq_lengths = checkpoint["custom"]["seq_lengths"]
                self.primitive_types = checkpoint["custom"]["primitive_types"]
                self.all_rules = serialization.tuplify(
                    checkpoint["custom"]["all_rules"]
                )
                self.rules_mask = checkpoint["custom"]["rules_mask"]

                print("use grammar production rules from checkpoint file")

                return

        print("use grammar production rules from preprocessed data files")
...
class NL2CodeDecoder(torch.nn.Module):
    ...
    def __init__(
        ...
        self.value_vocab_path = value_vocab_path
        self.value_vocabs = None

        if use_align_mat:
            from source.text2sql.ratsql.models.spider import spider_dec_func

            self.compute_align_loss = lambda *args: spider_dec_func.compute_align_loss(
                self, *args
            )
            self.compute_pointer_with_align = (
                lambda *args: spider_dec_func.compute_pointer_with_align(self, *args)
            )
        ...
        # TODO batching
        self.zero_rule_emb = torch.zeros(1, self.rule_emb_size, device=device)
        self.zero_recurrent_emb = torch.zeros(1, self.recurrent_size, device=device)
    ...
    def label_smooth_loss(self, X, target, smooth_value=0.1):
        if self.training:
            ...
            one_hot = torch.full(
                X.size(),
                smooth_value / (size - 1 if size > 1 else size),
                device=X.device,
            )
    ...
    def compute_loss(self, enc_input, example, desc_enc, debug):
        if not (self.enumerate_order and self.training):
            result = self.compute_mle_loss(enc_input, example, desc_enc, debug)
        else:
            result = self.compute_loss_from_all_ordering(
                enc_input, example, desc_enc, debug
            )

        if debug:
            mle_loss, history = result
        else:
            mle_loss = result

        if self.use_align_loss:
            align_loss = self.compute_align_loss(desc_enc, example)
            loss = mle_loss + align_loss
        else:
            loss = mle_loss
    ...
    def compute_mle_loss(self, enc_input, example, desc_enc, debug=False):
        traversal = TrainTreeTraversal(
            self, desc_enc, debug, enc_input["columns"], enc_input["tables"]
        )
        traversal.step(None)
        queue = [
            TreeState(
    ...
    def _update_state(
        ...
        desc_context, attention_probs = self._desc_attention(prev_state, desc_enc)
        ...
        return new_state, attention_probs
    ...
    def apply_rule(
        ...
        new_state, attention_probs = self._update_state(
            node_type,
            prev_state,
            prev_action_emb,
            parent_h,
        ...
        return output, new_state, rule_logits, attention_probs
    ...
    def gen_token_loss(self, output, gen_logodds, token, desc_enc):
        ...
        if desc_locs:
            # copy: if the token appears in the description (NL question) at least once
    ...
    def get_value_vocabs(self):
        if self.value_vocabs:
            return self.value_vocabs

        # Get value vocabs
        if self.value_vocab_path:
            with open(self.value_vocab_path, "r") as f:
                value_vocabs = json.load(f)
            self.value_vocabs = value_vocabs
            self.value_vocabs += [str(w) for w in self.terminal_vocab]
            return set(self.value_vocabs)
        else:
            return set()

    def _get_value(self, toks):
        vocabs = self.get_value_vocabs()
        values = []
        tmp = []
        for tok in toks:
            if tok in vocabs or '"tok"' in vocabs:
                tmp.append(tok)
            elif tok[-1] == "년":
                tmp.append(tok)
            elif tok[-1] == "월":
    ...
    def token_infer(self, output, gen_logodds, desc_enc, text):
        # Copy tokens
        # log p(copy | output)
        # shape: batch (=1)
        ...
        accumulate_logprobs(
            log_prob_by_word, zip(desc_enc.words, copy_loc_logprobs.squeeze(0))
        )
        ...
        # if not self.training and any([word in self.terminal_vocab for word in text]):
        #     # Mask tokens not in the NL query
        #     for word, word_logprob in log_prob_by_word.items():
        #         if word not in text and word not in [vocab.EOS, vocab.BOS, vocab.UNK, False, True]:
        #             log_prob_by_word[word] = torch.tensor(float('-inf'), device=word_logprob.device)
