...
from source.text2sql.ratsql.models.nl2code import decoder
...
class TreeTraversal:
    ...
    def __init__(self, model, desc_enc):
        ...
        self.prev_action_emb = model.zero_rule_emb.to(model._device)
        ...
        self.cur_item = TreeTraversal.QueueItem(
            ...
            state=initial_state,
            node_type=root_type,
            parent_action_emb=self.model.zero_rule_emb.to(model._device),
            parent_h=self.model.zero_recurrent_emb.to(model._device),
    ...
    def process_sum_inquire(self, last_choice):
        ...
        output, self.recurrent_state, rule_logits, attention_probs = (
            self.model.apply_rule(
                self.cur_item.node_type,
        ...
        self.update_prev_action_emb = TreeTraversal._update_prev_action_emb_apply_rule
        choices = self.rule_choice(
            self.cur_item.node_type, rule_logits, attention_probs
        )
    ...
    def process_list_length_inquire(self, last_choice):
        ...
        self.update_prev_action_emb = TreeTraversal._update_prev_action_emb_apply_rule
        choices = self.rule_choice(list_type, rule_logits, attention_probs)
        return choices, False
    ...
    def process_pointer_inquire(self, last_choice):
        # a. Ask which one to choose
        output, self.recurrent_state, logits, attention_probs, memory_pointer_probs = (
        ...
        choices = self.pointer_choice(
            self.cur_item.node_type, logits, attention_probs, memory_pointer_probs
        )
        return choices, False
    ...
    def rule_choice(self, node_type, rule_logits, attention_probs=None):
