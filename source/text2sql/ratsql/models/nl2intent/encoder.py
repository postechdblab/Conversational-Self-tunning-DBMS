...
@registry.register("encoder", "NL2Intent")
class NL2IntentEncoder(torch.nn.Module):
    ...
    def __init__(
        self, device, preproc, word_emb_size=128, recurrent_size=256, dropout=0.0
    ):
        super().__init__()
        self._device = device
        ...
        self.desc_embedding = torch.nn.Embedding(
            num_embeddings=len(self.desc_vocab), embedding_dim=self.word_emb_size
        )
    ...
    def forward(self, desc_words):
        ...
        desc_indices = torch.tensor(
            self.desc_vocab.indices(desc_words), device=self._device
        ).unsqueeze(0)
        ...
        return NL2IntentEncoderState(
            state=state, memory=outputs.transpose(0, 1), words=desc_words
        )
