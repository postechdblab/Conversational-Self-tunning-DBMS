...
POSTGRES_GRAMMAR_IDENTIFIERS = {"postgres", "postgresql"}
...
@registry.register("grammar", "postgres")
class PostgresLanguage:
    ...
    def unparse(self, tree, item, refine_from=True):
        unparser = PostgresUnparser(
            self.ast_wrapper,
            item.schema,
            self.factorize_sketch,
            do_refine_from=refine_from,
        )
    ...
    def parse_col_unit(self, col_unit):
        agg_id, col_ids, is_distinct = col_unit
        assert isinstance(col_ids, list), "col_ids should be list type!"
        result = {
            "_type": "col_unit",
                    ...
                    **(
                        {
                            "from": self.parse_from(
                                sql["from"], self.infer_from_conditions
                            ),
                        }
                        if self.output_from
                        else {}
        ...
        return filter_nones(
            {
                ...
                "conds": (
                    self.parse_cond(from_["conds"], optional=True)
    ...
    COND_TYPES_F, COND_TYPES_B = bimap(
        ...
        range(1, 11),
        ("Between", "Eq", "Gt", "Lt", "Ge", "Le", "Ne", "In", "Like", "Is"),
    )

    UNIT_TYPES_F, UNIT_TYPES_B = bimap(
        # ('none', 'unnest', 'array_length', '-', '+', '*', '/'),
        range(7),
        ("Column", "Unnest", "ArrLen", "Minus", "Plus", "Times", "Divide"),
    )
...
@attr.s
class PostgresUnparser:
    ast_wrapper = attr.ib()
    schema = attr.ib()
    factorize_sketch = attr.ib(default=0)
    do_refine_from = attr.ib(default=True)
    ...
    def unparse_val(self, val):
        ...
        if val["_type"] == "Null":
            return "null"
        if val["_type"] == "String":
            return "'" + val["s"].strip("'") + "'"  # Wrap with single quotes
        if val["_type"] == "Time":
            return f"current_timestamp + {val['t']}"
    ...
    def unparse_col_unit(self, col_unit):
        if "concat_columns" in col_unit:
            concat_columns = col_unit["concat_columns"]
            _column_names = []
            for concat_column in concat_columns:
                col_id = concat_column["col_id"]
                column = self.schema.columns[col_id]
                if column.table is None:
                    _column_name = column.orig_name
                else:
                    _column_name = f"{column.table.orig_name}.{column.orig_name}"
                _column_names.append(_column_name)
            column_name = '||"_"||'.join(_column_names)
        else:
    ...
    def unparse_val_unit(self, val_unit):
        ...
        elif val_unit["_type"] == "Unnest":
            col1 = self.unparse_col_unit(val_unit["col_unit1"])
            return f"unnest({col1})"
        elif val_unit["_type"] in {"ArrLen"}:
            col1 = self.unparse_col_unit(val_unit["col_unit1"])
            dim = val_unit["dim"]
            return f"array_length({col1}, {dim})"
        else:
            col1 = self.unparse_col_unit(val_unit["col_unit1"])
    ...
    def unparse_cond(self, cond, negated=False):
        ...
        if cond["_type"] == "Is":
            tokens.append(self.COND_TYPES_B[cond["_type"]])
            if negated:
                tokens.append("NOT")
            tokens.append(self.unparse_val(cond["val1"]))
        else:
            if negated:
                tokens.append("NOT")
