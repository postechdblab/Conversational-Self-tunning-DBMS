...
from source.text2sql.ratsql.datasets import spider
from source.text2sql.ratsql.utils import registry
from source.text2sql.wikisql.lib import dbengine
from source.text2sql.wikisql.lib import query
        ...
        for line in open(path):
            ...
            columns = tuple(
                ...
                for i, (col_name, col_type) in enumerate(
                    zip(schema_dict["header"], schema_dict["types"])
                )
            )
    ...
    class Metrics:
        ...
        def finalize(self):
            ...
            return {
                "per_item": [
                    {"ex": ex, "lf": lf}
                    for ex, lf in zip(self.exec_match, self.lf_match)
                ],
