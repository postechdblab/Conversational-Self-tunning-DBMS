...
class Evaluator:
    ...
    def __init__(
        self, db_dir, kmaps, tables, etype, db_type="sqlite", append_db_id=False
    ):
        self.db_dir = db_dir
        self.kmaps = kmaps
        self.tables = tables
        ...
        for db_name in self.kmaps.keys():
            if "global_schema" in db_name:
                continue
            db_conn_str = db_utils.create_db_conn_str(db_dir, db_name, db_type=db_type)
            self.db_paths[db_name] = db_conn_str
            self.schemas[db_name] = Schema(
                get_schema(db_conn_str, db_type=db_type, append_db_id=append_db_id)
            )
    ...
    @classmethod
    def eval_exact_match(cls, pred, label, partial_scores=None):
        if not partial_scores:
            partial_scores = cls.eval_partial_match(pred, label)
            cls.partial_scores = partial_scores
