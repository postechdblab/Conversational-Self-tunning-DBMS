from source.text2sql.wikisql.lib.common import detokenize
...
class Query:
    ...
    def __eq__(self, other):
        if isinstance(other, self.__class__):
            indices = (
                self.sel_index == other.sel_index and self.agg_index == other.agg_index
            )
            if other.ordered:
                conds = [
    ...
    def from_sequence(cls, sequence, table, lowercase=True):
        ...
        while where_terms:
            ...
            flat = flatten(where_terms)
            val_end_index = (
                flat["words"].index("symand")
                if "symand" in flat["words"]
                else len(where_terms)
            )
